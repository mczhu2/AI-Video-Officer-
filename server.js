...
function buildInstructions(session, mode) {
  ...
  return base.concat([
    ...
    '招聘目标是普通快递员，不是优秀快递员；评估要务实、宽松，重点看是否有相关经验、是否能正常表达、是否有基本处理思路。',
    '如果候选人说做过快递、外卖、同城配送、仓配等相关工作，并能说清基本做法，就应倾向认为基本可以。',
    '不要因为回答不够优秀、不够完整、没有复杂案例或没有高峰极限数据就给负面评价；这些只写成后续确认点。',
    '严禁输出“健康：”“异常：”“通过：”“未通过：”“动作正常”或“内容异常”这类动作判断结论。',
    '即使候选人内容很少、经历不清楚，也必须输出结构化评估，而不是输出异常标签；信息不足时写“可后续补充确认”，不要直接淘汰。',
    '请严格按照以下三行格式输出，不要增加任何别的行：',
    '概述：一句话概括候选人回答了什么。',
    '判断：按普通快递员上岗口径判断是否有基础经验、表达是否正常、处理思路是否基本可用。',
    '风险：只写需要后续确认或培训的点；不要把“不够优秀”写成不适合。',
  ]).join('\n');
}
  ...
  evaluationSocket.on('open', () => {
    ...
    send({
      ...
      session: {
        ...
        temperature: 0.2,
        max_tokens: 480,
        instructions: '你是快递员岗位终面评估官，请按普通快递员上岗口径宽松、务实地输出结构化最终结论。',
      },
    });
    send({
      type: 'conversation.item.create',
