...
function buildQuestionFallback(step, transcript, raw) {
  ...
  return {
    ...
    summary: short ? '回答内容较少，但可作为基础表达记录。' : normalizedTranscript,
    judgement: short ? '回答较简短；普通快递员岗位可先结合其他题目和试岗继续判断。' : '回答包含基础信息；普通快递员岗位可视为具备初步沟通表达。',
    risk: raw
      ? '模型返回格式不完整，已回退为宽松上岗口径解析；不直接作为淘汰依据。'
      : '信息有限，建议后续补充确认或试岗观察，不直接作为淘汰依据。',
    raw: String(raw || '').trim(),
  };
}
...
function buildCourierInterviewReport(report) {
  ...
  const finalReport = {
    ...
    recommendation: allActionsPass && allQuestionsAnswered
      ? '基本符合普通快递员岗位要求，建议进入下一轮或安排试岗；后续重点确认出勤稳定性和路线熟悉度。'
      : '暂未收集完整信息，建议补充健康动作或关键问答后再判断；不要仅因表达不够优秀直接淘汰。',
    summary: allActionsPass && allQuestionsAnswered
      ? '候选人已完成快递员基础面试，具备普通快递员岗位所需的基础健康配合和岗位经验线索。'
      : '快递员面试信息尚不完整，需补充核验；评估应按普通快递员上岗口径处理。',
  };
